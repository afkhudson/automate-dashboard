...
#open data dump FILE
datadump = r"C:/Users/sosa/OneDrive - Razer (Asia-Pacific) Pte. Ltd/Razer/Worksheets/Dashboard/Tools/MS - BBY - Data Dump - Copy.xlsx"
...
#get latest AMZ report
amzraw_dir = r"Amazon Raw/*"
amz_list_of_files = glob.glob(amzraw_dir)
amzraw_latest = max(amz_list_of_files, key=os.path.getctime)
print(f'amzraw_latest created: {time.ctime(os.path.getmtime(amzraw_latest))}')

amzraw_df = pd.read_excel(open(amzraw_latest,'rb'),skiprows=2)
amzraw = amzraw_df
print(f'rows: {len(amzraw)} columns: {len(amzraw.columns)}')
...
sleep(2)
xlbook = xlapp.Workbooks.Open("C:/Users/sosa/OneDrive - Razer (Asia-Pacific) Pte. Ltd/Razer/Worksheets/Dashboard/Tools/MS - BBY - Data Dump - Copy.xlsx")
xlapp.DisplayAlerts = False
xlapp.Visible = True
print('opening datadump to refresh')
xlbook.RefreshAll()
print('refreshing pivottables')
input('After checking Datadump for latest SKUs, press any key')
xlbook.Save()
xlapp.Quit()
...
bbyCleanData = bbydf.iloc[:, [8,9,10,11,12,]]
...
#clean data
amzCleanData = amzraw.iloc[:, lambda df:[0,1,5,6,7]]
#keeps only amz and amz canada rows
amzCleanData = amzCleanData.loc[(amzCleanData['Unnamed: 0'] == 'Amazon') | (amzCleanData['Unnamed: 0'] == 'Amazon Canada')]


#grouping clean data
accountsdf = [msCleanData, bbyCleanData, spaCleanData, bhCleanData, adoramaCleanData, amzCleanData]


#creating excel sheet with todays date
writer = pd.ExcelWriter(f'C:/Users/sosa/OneDrive/Coding/automate-dashboard/Compiled Report/{d4} sell thru data.xlsx')
...
writer.save()
writer.handles = None
xw.Book(f'C:/Users/sosa/OneDrive/Coding/automate-dashboard/Compiled Report/{d4} sell thru data.xlsx')
